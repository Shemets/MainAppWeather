...
//    }
    ...
    private func setupUI() {
        ...
        mainForecastView.frame = CGRect(x: view.bounds.midX - mainForecastWidth / 2,
                                        y: view.bounds.midY - mainForecastHeight * 2,
    ...
    private func setupUIMainForecast() {
        ...
        tempLabel.frame = nameCityLabel.frame.offsetBy(dx: 0, dy: labelHeight)
        tempLabel.backgroundColor = .clear
        tempLabel.text = "Loading.."
        tempLabel.textColor = .white
        tempLabel.font = .systemFont(ofSize: 60)
        tempLabel.textAlignment = .center
        mainForecastView.addSubview(tempLabel)
        
        let imageWidth: CGFloat = 70
        let imageHeight: CGFloat = 70
        imageViewForecast.frame = CGRect(x: mainForecastView.bounds.midX + imageWidth / 3,
                                         y: mainForecastView.bounds.minY + imageHeight / 1.5  ,
                                         width: imageWidth,
                                         height: imageHeight)
        imageViewForecast.backgroundColor = .clear
        mainForecastView.addSubview(imageViewForecast)
        
        textForecastLabel.frame = tempLabel.frame.offsetBy(dx: 0, dy: labelHeight)
...
private func updateWeatherInfo(latitude : Double, longtitude : Double) {
    let stringURL = "https://api.openweathermap.org/data/2.5/onecall?lat=\(latitude.description)&lon=\(longtitude.description)&exclude=minutely&appid=8f1bd7e940447af928cf9025cae58514&units=metric"
